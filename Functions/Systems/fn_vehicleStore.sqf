...
if (_impound) exitWith {
    if (count _vInfo isEqualTo 0) then  {
        ...
        _query = format ["UPDATE vehicles SET active='0', fuel='%3', damage='%4' WHERE active='1' pid='%1' AND plate='%2'",_uid , _plate, _fuel, _damage];
...
// update
_query = format ["UPDATE vehicles SET active='0', inventory='%3', gear='%4', fuel='%5', damage='%6' WHERE pid='%1' AND plate='%2' AND active='1'", _uid, _plate, _trunk, _cargo, _fuel, _damage];
_thread = [_query,1] call DB_fnc_asyncCall;
